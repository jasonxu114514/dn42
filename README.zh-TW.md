...
Backend 可以與 bot 跑在同一台主機上。Agent 通常放在各 PoP 的路由器上，一個 PoP 一個 agent。每個 agent 都有自己的 URL 與 bearer token，由 admin panel 管理。
...
4. Backend 呼叫 agent 的 `POST /v1/peers/deploy`。
...
Endpoint 的主機部分來自 admin panel 中註冊的 agent URL，backend 會加上由 ASN 推導出的 WireGuard listen port。
...
Backend 初始沒有任何 agent。請在 **Admin > Agents** 註冊每個 PoP，然後把產生的 token 複製到對應路由器的 `config.json`。Backend 會從 agent 的 `GET /v1/pubkey` 抓取 `wireguard_public_key`，並顯示給 peer 使用。
...
| Key | 預設值 | 用途 |
| --- | --- | --- |
| `listen` | `:8080` | Agent 監聽地址。 |
| `token` | 空 | 每個 route 需要的 bearer token。空值表示不啟用 auth。 |
...
- Agent 設定 `token` 後，每個 route 都需要 `Authorization: Bearer <token>`。
...
| Peer 設定顯示 `<our-wireguard-public-key>` | 到 **Admin > Agents** refresh pubkey，並確認 agent 設定了 `wireguard_public_key`。 |
