...
- **Agent**：在每台路由器上以 root 執行的 Go 服務，負責執行固定參數的 looking-glass 指令，並套用每個 peer 的 WireGuard 與 BIRD 設定。
- **Telegram bot**：提供登入、建立／編輯／刪除 peer、查詢 peer 狀態與 looking-glass 指令。

使用者驗證 ASN 後，可以選擇 PoP、提交自己的 WireGuard endpoint 與公鑰，並取得建立隧道所需的我方參數。Backend 會產生 WireGuard 與 BIRD 片段，再送到對應路由器上的 agent。
...
| - 驗證 ASN 擁有權                               |
| - 儲存 users, agents, peers, sessions           |
...
Backend 可以與 bot 跑在同一台主機上。Agent 通常放在各 PoP 的路由器上，一個 PoP 一個 agent。每個 agent 會以 bearer token 主動建立 WSS 長連線到 backend；admin panel 中的 URL 則作為產生 WireGuard 設定時使用的公開 endpoint 主機。
...
2. 使用者在 Web portal 或 Telegram bot 建立 peer：
   - 選擇啟用中的 PoP；
   - 輸入自己的 WireGuard endpoint，格式為 `host:port`；
   - 輸入自己的 WireGuard public key；
   - 選擇 WireGuard MTU，預設 `1420`；
   - Web UI 可另外覆寫 link-local BGP 位址。
3. Backend 驗證所有欄位，套用每個 ASN 在每個 PoP 只能有一個 peer 的規則，將 peer 自動核准，並產生 WireGuard 與 BIRD 設定。
...
6. 部署成功後，Web UI 與 bot 會顯示使用者設定自己端點所需的我方參數：
   ...
   - 該 PoP 的我方 WireGuard public key；
...
Endpoint 的主機部分來自 admin panel 中註冊的 endpoint URL，backend 會加上由 ASN 推導出的 WireGuard listen port。
...
Backend 初始沒有任何 agent。請在 **Admin > Agents** 註冊每個 PoP，然後把 PoP name、產生的 token 與 backend WSS URL 複製到對應路由器的 `config.json`。Agent 連線後會透過 WSS 回報 heartbeat/system status、整台伺服器的 WireGuard/BIRD 狀態與 `wireguard_public_key`，backend 會快取並顯示給 peer 使用。
...
| Key | 預設值 | 用途 |
| --- | --- | --- |
| `name` | 空 | 與 backend agent record 相同的 PoP 名稱；使用 WSS 時必填。 |
...
| `max_concurrency` | `4` | 同時執行的 looking-glass/status 指令數。`0` 表示不限制。 |
...
| `/portal` | 使用者建立 peer、查看部署狀態與產生的設定。 |
| `/admin` | 操作者總覽。 |
| `/admin/agents` | 註冊／編輯 PoP、更新 pubkey、重設 token。 |
| `/admin/peers` | 編輯、redeploy、停用、刪除 peer。 |
| `/admin/users` | 管理 users 與 Telegram bindings。 |
| `/admin/lg-log` | Looking-glass 查詢紀錄。 |

Portal 建立 peer 時可設定 WireGuard MTU。Admin 的 peer 表格也可編輯 MTU，方便修正後 redeploy。
...
/delete       引導式刪除 peer
/ping         從某個 PoP 執行 ping
/trace        從某個 PoP 執行 traceroute
/mtr          從某個 PoP 執行 mtr
/route        從某個 PoP 執行 BIRD route lookup
/status       從某個 PoP 執行 BIRD status 查詢
/cancel       中止目前引導流程
```

`/create` 會詢問 PoP、endpoint、public key 與 MTU。MTU 步驟輸入 `default` 會使用 `1420`。`/edit` 的 MTU 步驟輸入 `keep` 會保留目前值。
...
| Agent 一直顯示 offline | 檢查 `name`、`backend_wss_url`、TLS 連線，以及 token 是否與 **Admin > Agents** 相同。 |
| Agent 回傳 `unauthorized` | Backend 裡的 agent token 與 agent `config.json` 的 token 不一致。 |
| Peer 設定顯示 `<our-wireguard-public-key>` | 讓 agent 連上 WSS 或到 **Admin > Agents** refresh pubkey，並確認 agent 設定了 `wireguard_public_key`。 |
