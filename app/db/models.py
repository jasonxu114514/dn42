...
class Node(Base):
    ...
    node service dials the control plane over WSS. Renamed from ``Node service`` — the on-router daemon is still
    "the node service", but the control-plane concept is a Node.
    節點(PoP):由操作者經營、可經 WireGuard 連線的路由器,其上的 Go node service 以 WSS 連回控制平面。"""
    ...
    # peer's ASN. (The node service's own control channel is the WSS it dials out on, not this field.)
    url: Mapped[str] = mapped_column(String(512))
    token: Mapped[str] = mapped_column(String(255), default="")
    # Our WireGuard public key on this Node, reported by the node service over WSS and shown to peers in
    # their generated config. Empty until the first heartbeat/pubkey refresh.
    # 本節點的 WireGuard 公鑰,由 node service 透過 WSS 回報,並填入對等端產生的設定。首次同步前為空。
...
class PeerRequest(Base):
    ...
    peer_dn42_ipv4: Mapped[str] = mapped_column(String(64), default="")
    peer_dn42_ipv6: Mapped[str] = mapped_column(String(64), default="")
    bgp_extended: Mapped[bool] = mapped_column(Boolean, default=True)
