...
# 請勿改回僅限 dn42 範圍。公開 LG 的濫用由每 IP 速率限制與 node service 併發上限約束，而非目標位址。
ALLOWED_NETWORKS = (
    ipaddress.ip_network("0.0.0.0/0"),
    ipaddress.ip_network("::/0"),
)
# Reject shell/format metacharacters so a target can never break out of the fixed argv on the node service.
# 拒絕 shell／格式化中介字元，使目標無法跳脫 node service 端的固定 argv。
...
# node service; since the LG already allows any resolved address, accepting a hostname does not widen the
...
# 至少含一個點,總長 ≤ 253。ping/traceroute 於 node service 端自行解析；
...
def validate_target(query_type: str, target: str) -> str:
    ...
    try:
        ...
        # Not an IP — accept a valid hostname (ping/traceroute resolve it on the node service).
