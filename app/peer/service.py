...
def _load_peer_for_node_service(db, peer: PeerRequest) -> PeerRequest:
...
def create_peer(
    ...
    *,
    ...
    This function commits. The row is visible as ``deploying`` before the synchronous node call,
...
def delete_peer(db, *, peer: PeerRequest) -> None:
    ...
    This function commits. The node teardown uses a snapshot after the read transaction is closed.
    """
    peer = _load_peer_for_node_service(db, peer)
    peer_snapshot, node_snapshot = _snapshot_peer(peer)
    db.commit()
