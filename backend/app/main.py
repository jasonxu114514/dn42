...
def portal(request: Request, db: Session = Depends(get_db)) -> HTMLResponse:
    ...
    return render(request, "portal.html", {"nodes": nodes, "peers": peers})
...
def peer_config(peer_id: int, request: Request, db: Session = Depends(get_db)) -> HTMLResponse:
    ...
    return render(
        request,
        "config.html",
        {"title": f"Peer #{peer.id} config", "config": render_user_config(peer, peer.node)},
    )
...
def admin_peer_action(
    ...
    if peer is None:
        raise HTTPException(status_code=404, detail="Peer request not found")
    peer.status = "approved" if action == "approve" else "rejected"
...
def admin_peer_config(peer_id: int, request: Request, db: Session = Depends(get_db)) -> HTMLResponse:
    ...
    return render(
        request,
        "config.html",
        {"title": f"Operator config for request #{peer.id}", "config": render_operator_config(peer, peer.node)},
    )
