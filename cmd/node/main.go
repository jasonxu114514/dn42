...
import (
	...
	"dn42-autopeer-node/internal/api"
	"dn42-autopeer-node/internal/backend"
...
func main() {
	...
	// 絕對路徑。
	configPath := flag.String("config", "config.json", "path to the node config JSON file")
	flag.Parse()
	...
	// The node's public key is required and reported to the control plane so each peer can
	...
	// node service 的公鑰為必填,並回報給控制平面,讓每個對等端都能取得完整設定。
