...
The node service is the router-side service used by the backend over an node-initiated WSS connection. It
has two jobs:

- run looking-glass and peer-status commands with fixed argv;
- deploy or remove generated WireGuard and BIRD peer config.

The node service does not perform ASN authentication. It trusts the authenticated backend connection, so
...
Every node connection should use:

```http
Authorization: Bearer <token>
```

The token is checked by the backend in constant time against the token stored on the node record.

### WSS transport

For normal backend communication, the node service connects to:

```text
wss://<backend>/api/nodes/ws?name=<node-name>
Authorization: Bearer <token>
```

If `backend_wss_url` is configured as a bare backend `http(s)` URL, the node service converts it to
`ws(s)` and uses `/api/nodes/ws`. The `name` value must match the node record in Admin > Nodes.

After connecting, the node service sends:
...
`ok` means the requested operation completed successfully. `applied` means the node service already wrote
...
node service returns `{"ok": false, "output": "node is busy, try again shortly"}` instead of queueing work.

## `pubkey`

Returns the node service's configured `wireguard_public_key`.
...
The node service validates this key during startup and refuses to start if it is missing or malformed. The
...
Accepts the same target shape as ping. For IPv6 literal targets, the node service adds `-6`.
...
literal targets, the node service adds `-6`.
...
The node service runs two commands inside one concurrency slot:
...
- MTU is part of `wireguard_config`; there is no separate node-side MTU JSON field.
...
- The node service replaces `{{WIREGUARD_PRIVATE_KEY}}` with `wireguard_private_key` from its config.
...
node service returns `ok: false` and appends `command timed out` to the command output.
...
node connection when users create, edit, redeploy, disable, or delete peers. It also sends WSS
