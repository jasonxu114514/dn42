// Package config loads the node service's runtime configuration from a single JSON file. It replaces the
...
// config 套件從單一 JSON 檔載入 node service 的執行設定，取代原本的環境變數設定:所有設定(含 bearer token
...
// Config 為 node service 的完整執行設定。欄位預設值於解碼後由 applyDefaults 套用,因此最小設定檔只需給定
// wireguard_public_key(必填),通常再加上 token 即可。
type Config struct {
	Name  string `json:"name"`
	Token string `json:"token"`
	// BackendWSURL is the backend websocket endpoint this node service connects to, usually
	// wss://<backend>/api/nodes/ws. It is required; the node service no longer starts a listener.
	...
	// "Permission denied". The node service sets only the group (mode stays 0750/0640, never world-readable).
	...
	// BirdPeerGroup 為應擁有 BIRD 對等目錄與各對等片段檔(由 node service 寫入)的群組。node service 以 root 執行,但
	// BIRD 守護程序以非特權身分(通常是 `bird` 使用者)執行,否則無法讀取 root 擁有的片段,使
	// `birdc configure` 以「Permission denied」失敗。node service 僅設定群組(權限位維持 0750/0640,絕不全域
